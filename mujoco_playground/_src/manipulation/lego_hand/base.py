...
class LegoHandEnv(mjx_env.MjxEnv):
  """Base class for LEGO hand environments."""
  ...
  def get_fingertip_positions(self, data: mjx.Data) -> jax.Array:
    ...
    return jp.concatenate([
        mjx_env.get_sensor_data(self.mj_model, data, f"f{i}_tip_position")
        for i in range(1, 6)
    ])
